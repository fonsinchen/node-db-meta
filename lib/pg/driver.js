...
Driver.prototype.getColumns = function (tableName, callback) {
  ...
  function checkAutoincrement(err, columns, client) {
    ...
    return client.query("SELECT c.relname FROM pg_class c WHERE c.relkind = 'S';",
...
Driver.prototype.getForeignKeys = function(tableName, callback) {
  var columns = {};
  var self = this;
  var queryColumnPositions = function(tableName, callback) {
    if (columns[tableName] !== undefined) {
      callback(columns[tableName]);
    } else {
      self.client.query("SELECT table_name, column_name, ordinal_position " + 
        ...
        result.rows.forEach(function (row) {
          columns[tableName][row.ordinal_position] = row.column_name;
        });
        return callback(null, columns[tableName]);
      });
    }
  };

  var sql = "SELECT c.conname AS name, c.conkey AS columns, " +
      "t2.relname AS foreign_table_name, c.confkey AS foreign_columns " +
      "FROM pg_constraint c LEFT JOIN pg_class t ON c.conrelid  = t.oid " + 
      "LEFT JOIN pg_class t2 ON c.confrelid = t2.oid " + 
      "WHERE c.contype='f' and t.relname= $1";
  return self.client.query(sql, [tableName], function(err, result) {
    if (err) return callback(err);
    var objects = [];
    return queryColumnPositions(tableName, function(err, columns) {
      if (err) {
        return callback(err);
        ...
        return result.rows.forEach(function(row) {
          ...
          queryColumnPositions(row.foreign_table_name, function(err, fcolumns) {
            if (err) return callback(err);
            row.column_names = [];
